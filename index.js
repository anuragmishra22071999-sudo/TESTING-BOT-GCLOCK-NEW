...
app.get('/', (req, res) => {
    ...
    <html>
    <head>
        <title>Anurag Bot Configuration</title>
        <style>
    ...
    <body>
        <h1>🤖 Anurag Bot Configuration</h1>
        <form id="configForm">
            ...
            <div class="form-group">
                ...
                <input type="text" id="prefix" name="prefix" value="/anurag" required>
            </div>
            <div class="form-group">
...
app.post('/configure', (req, res) => {
    try {
        ...
        config.prefix = req.body.prefix || '/anurag';
...
app.listen(PORT, () => {
    console.log(`🔥 Anurag Bot running on port ${PORT}`);
    try {
...
async function initializeBot() {
    ...
    console.log('⚡ Initializing Anurag Bot with WS3-FCA...');
    ...
    try {
        ...
        console.log('✅ Anurag Bot is now listening for events...');
    } catch (err) {
...
async function handleGroupCommand(api, event, args, isAdmin) {
    ...
    if (subCommand === 'on') {
        ...
        await api.sendMessage(`🔒 Group name locked to: ${groupName}`, threadID);
    }
}
...
async function handleNicknameCommand(api, event, args, isAdmin) {
    ...
    if (subCommand === 'on') {
        ...
        await api.sendMessage(`🔒 All nicknames locked to: ${nickname}`, threadID);
    }
}
...
async function handleFightCommand(api, event, args, isAdmin) {
    ...
    const subCommand = args.shift();
    if (subCommand === 'on') {
        fightSessions[threadID] = { active: true, target: null };
        await api.sendMessage("⚔️ Enter hater's name:", threadID);
    }
}
...
async function handleThreadNameChange(api, event) {
    ...
    if (lockedGroups[threadID] && authorID !== config.adminID) {
        ...
        await api.sendMessage({
            body: "❌ Group name is locked! Only admin can change it.",
            mentions: [{ tag: "@user", id: authorID }]
        }, threadID);
    }
}
...
async function handleNicknameChange(api, event) {
    ...
    if (lockedNicknames[threadID] && authorID !== config.adminID) {
        ...
        await api.sendMessage({
            body: "❌ Nicknames are locked! Only admin can change them.",
            mentions: [{ tag: "@user", id: authorID }]
        }, threadID);
    }
